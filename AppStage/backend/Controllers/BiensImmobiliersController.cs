...
public async Task<IActionResult> GetAllBiens(
    ...
    [FromQuery] int? proprietaireId = null)
{
    var biens = await _bienService.GetAllBiensAsync(recherche, typeDeBienNom, statut, ville, quartier, prixMin, prixMax, triParPrix, triParDate, dateDebut, dateFin, nombreVoyageurs, proprietaireId);
    return Ok(biens);
}
