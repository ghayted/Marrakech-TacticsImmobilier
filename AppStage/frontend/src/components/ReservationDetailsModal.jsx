...
const ReservationDetailsModal = ({ isOpen, onClose, reservation, onCancel, onStatusUpdate }) => {
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://marrakech-tacticsimmobilier.onrender.com/api/Reservations/${reservation.id}/annuler`, {
        method: 'PUT',
        headers: {
  ...
  const handleStatusChange = async (newStatus) => {
    ...
    try {
      const token = localStorage.getItem('authToken');
      const response = await fetch(`https://marrakech-tacticsimmobilier.onrender.com/api/Reservations/${reservation.id}/statut`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
