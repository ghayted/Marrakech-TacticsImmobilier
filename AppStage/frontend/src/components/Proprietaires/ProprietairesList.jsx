...
const ProprietairesList = () => {
  ...
  const fetchProprietaires = async () => {
    try {
      setLoading(true);
      const response = await fetch('https://marrakech-tacticsimmobilier.onrender.com/api/proprietaires');
      if (response.ok) {
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Êtes-vous sûr de vouloir supprimer ce propriétaire ?')) {
      try {
        const response = await fetch(`https://marrakech-tacticsimmobilier.onrender.com/api/proprietaires/${id}`, {
          method: 'DELETE'
        });
        if (response.ok) {
