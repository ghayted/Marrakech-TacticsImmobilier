...
const ReservationPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchProperty = async () => {
      try {
        const response = await fetch(`https://marrakech-tacticsimmobilier.onrender.com/api/BiensImmobiliers/${id}`);
        if (response.ok) {
  ...
  const handlePaymentSuccess = async (paymentIntent) => {
    ...
    try {
      ...
      if (reservationResponse.ok) {
        ...
        const paymentResponse = await fetch('https://marrakech-tacticsimmobilier.onrender.com/api/Payments/confirm-payment', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${localStorage.getItem('authToken')}`
